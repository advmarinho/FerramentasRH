...
# 🔥 Código completo do seu app Streamlit embutido como string
streamlit_code = """
...
- A **carga de atendimentos por atendente**
- A **distribuição por tipo de atendimento**
- Realizar um **Drill Down**
\"\"\")

excel_file = "C:/_RPA/Email/atendimentos_gg_2025.xlsx"
...
if page == "Total por Atendentes":
    ...
    fig = px.bar(sorted_colab, x="Atendente", y="Total Atendimentos", text=labels,
                 color="Total Atendimentos", color_continuous_scale=color_scale,
    ...
    selected_categories = st.multiselect("Selecione as categorias disponíveis",
    ...
    if filtered_tipo.empty:
        ...
        fig2 = px.pie(sorted_tipo, values="Total", names="Tipo de Atendimento",
                      title="Distribuição por Tipo de Atendimento", hole=0.4,
    ...
    selected_analysts = st.multiselect("Selecione os Analistas", options=options_list, default=[])
...
st.download_button(label="Baixar arquivo Excel",
                   data=file_bytes,
...
def main():
    ...
    temp_script_path = os.path.join(temp_dir, "temp_app_streamlit.py")
    ...
    subprocess.Popen(["streamlit", "run", temp_script_path])
    time.sleep(2)
